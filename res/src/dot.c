...
ud_arr      *ud_tens_get_counter_axis(size_t len, ud_arr *axis)
{
    ...
    counter_axis = ud_arr_tset(ud_arr *, ud_arr_type_arr(), ud_arr_tinit(ud_arr_type_size_t(), len - ((ud_arr **)axis->val)[0]->len), ud_arr_tinit(ud_arr_type_size_t(), len - ((ud_arr **)axis->val)[0]->len));
...
{
    ud_arr  *shape_mult = ud_arr_tinit(ud_arr_type_arr(), 2);
    ...
    ((ud_arr **)shape_mult->val)[0] = ud_arr_tset(size_t, ud_arr_type_size_t(), m_a, n_a);
    ((ud_arr **)shape_mult->val)[1] = ud_arr_tset(size_t, ud_arr_type_size_t(), n_b, m_b);
    return (shape_mult);
}
...
ud_tensor   *ud_tens_dot(ud_tensor *a, ud_tensor *b, ud_arr *axis)
{
    ...
    size_t  axis_len = ((ud_arr **)axis->val)[0]->len;
    ud_arr *mem_shapes = ud_arr_tinit(ud_arr_type_size_t(), (tens_len - axis_len) * 2);
    ...
    ud_arr *new_dim_a = ud_arr_tinit(ud_arr_type_size_t(), tens_len);
    ud_arr *new_dim_b = ud_arr_tinit(ud_arr_type_size_t(), tens_len);
    ...
    size_t  *t_axisb_val = (size_t *)((ud_arr **)axis->val)[1]->val;
    size_t  *t_counteraxisa_val = (size_t *)((ud_arr **)counter_axis->val)[0]->val;
    size_t  *t_counteraxisb_val = (size_t *)((ud_arr **)counter_axis->val)[1]->val;
    for (ud_ut_count i = 0; i < (const size_t)(tens_len - axis_len); ++i)
    {
        *(t_new_dima_val++) = a->shape[*(t_counteraxisa_val++)];
        *(t_new_dimb_val++) = b->shape[*(t_counteraxisb_val++)];
    }
    for (ud_ut_count i = tens_len - axis_len; i < tens_len; ++i)
    {
        *(t_new_dima_val++) = a->shape[*(t_axisa_val++)];
        *(t_new_dimb_val++) = b->shape[*(t_axisb_val++)];
    }
    ...
    ud_tensor *dot_res = ud_tens_matrixdot(reshape_t_a, reshape_t_b);
