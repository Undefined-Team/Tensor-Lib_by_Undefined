...
static size_t       ud_get_index(size_t len, size_t *coord, size_t *new_dim, size_t *new_shape_mult)
{
    size_t index = 0;
...
ud_tensor       *ud_tens_transpose_cpy(ud_tensor *tensor, ud_size_t_a *new_dim)
{
    size_t shape_len = tensor->shape_len;
    size_t *t_new_dim = ud_error_manager(new_dim, shape_len);
